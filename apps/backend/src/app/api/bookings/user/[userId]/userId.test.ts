...
describe('API Route: /api/bookings/user/[userId] (GET)', () => {
  ...
  // ⭐ IMPORTANTE: params es ahora una PROMESA
  const MOCK_PARAMS = { params: Promise.resolve({ userId: MOCK_USER_ID }) };
  const MOCK_INVALID_PARAMS = { params: Promise.resolve({ userId: '' }) };
